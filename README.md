...
# start the Discord bot (requires DISCORD_TOKEN)
lncrawl discord
```
...
`lncrawl discord` runs a Discord bot with `/crawl`, `/search`, and `/status` slash commands.
`/crawl` posts progress updates — novel info, chapter download progress, artifact generation —
between stages instead of waiting silently.
