...
- [lncrawl/\_\_main\_\_.py](lncrawl/__main__.py) → `main()` in `lncrawl/__init__.py`: frozen
  ...
  (`version`, `config`, `sources`, `crawl`, `search`, `server`, `discord`, `app`, plus a
  `dev` group with the migration commands).
...
- `lncrawl discord` starts a Discord bot ([lncrawl/discord_bot/](lncrawl/discord_bot/),
  [commands/discord.py](lncrawl/commands/discord.py)) with `/crawl`, `/search`, `/status`
  slash commands. `/crawl` posts progress updates (novel info, chapter/image download
  progress, artifact generation) via `interaction.followup.send` between stages. Requires
  `DISCORD_TOKEN`; optionally `DISCORD_GUILD_ID` (instant guild command sync) and
  `DISCORD_MAX_FILE_SIZE_MB` (default 25) — larger files go to Sharry (`SHARRY_URL`/`SHARRY_USER`/`SHARRY_PASS`).
