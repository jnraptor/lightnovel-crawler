...
- [lncrawl/\_\_main\_\_.py](lncrawl/__main__.py) → `main()` in `lncrawl/__init__.py`: frozen
  ...
  hidden `dev` group with the migration commands).
- `discord` starts a Discord bot (`lncrawl/discord_bot/`) with `/crawl`, `/search`, `/status`
  slash commands. `/crawl` sends progress updates (novel info, chapter/image download
  ...
  `DISCORD_MAX_FILE_SIZE_MB` (default 25).
